...
int main(){
    ...
    if(indices.size() == 0){
        ...
        //for(int i = 0; i <= indices.size() - 1; i++){   //imprime os valores do vetor indices, inclusive o ultimo
           cout << indices.at(i) << " ";
        }
        cout << indices.at(indices.size() - 1) << endl; ///imprime o ultimo valor do vetor de indices, necessario apenas se nao colocar o "=" no for 
    }
}